...
def vectorize_text(X_train, X_test, max_features=5000, ngram_range=(1, 2)):
    ...
    print(f"   Vectorizing text (max_features={max_features}, ngram_range={ngram_range})...")
    
    vectorizer = TfidfVectorizer(max_features=max_features, ngram_range=ngram_range)
    X_train_vectorized = vectorizer.fit_transform(X_train)
    X_test_vectorized = vectorizer.transform(X_test)
    
    print(f"   Training shape: {X_train_vectorized.shape}, Test shape: {X_test_vectorized.shape}")
...
def train_single_model(model_name, model_config, X_train_vec, X_test_vec, y_train, y_test, 
                       X_test, vectorizer, config):
    """Train and log a single model."""
    print(f"\n{'='*80}")
    ...
    with mlflow.start_run(run_name=model_name) as run:
        ...
        mlflow.sklearn.log_model(
            ...
            artifact_path="model",
            signature=signature,
            registered_model_name="fraud-detector-model" if register_model else None
        )
...
def main():
    ...
    print("\n" + "="*80)
    print("🚀 EMAIL PHISHING DETECTION - MODEL TRAINING & COMPARISON")
    print("="*80)
    ...
    for model_name, result in results_summary.items():
        if 'error' not in result:
            row = {'Model': model_name, **result['metrics'], 'Run ID': result['run_id']}
    ...
    if comparison_df:
        ...
        best_model = comparison_table.loc[comparison_table['f1_score'].idxmax()]
        ...
        comparison_path = artifacts_dir / f"model_comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
